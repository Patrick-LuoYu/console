...
module.exports = {
  ...
  POLICY_REQUEST_CONTENT_TIP: '如果端口协议非 HTTP、HTTP2 或 gRPC，则按请求内容下发不可用。',
  PREFIX_MATCH: '前缀匹配',
  ...
  BLUE_GREEN_DEPLOYMENT_DESC: '蓝绿部署在保留旧版本的同时部署新版本，确保零宕机。两个版本中总有一个版本处于在线状态接收所有流量，另一个版本则保持待机，如果有问题，可以快速回滚至旧版本。',
  CANARY_RELEASES_DESC: '金丝雀发布将一部分实际流量引入一个新版本进行测试，测试新版本的性能和表现，在保证系统整体稳定运行的前提下，尽早发现新版本在实际环境上的问题。',
  TRAFFIC_MIRROR_DESC: '流量镜像用来更为真实地测试新版本，提前发现问题，同时不对生产环境产生影响，从而提高版本发布的安全性与可靠性。',
  ...
  GRAY_RELEASE_BY_CONTENT_TIP: '根据请求内容配置规则，只有请求内容中满足特定条件的流量会切分到灰度版本上。该策略只对直接访问入口服务有效。',
  MIRROR_POLICY_DESC: '流量镜像将生产环境的流量复制到灰度版本中，在新版本上线到真实环境之前使用实时用户流量对它进行测试。</br>因此，流量镜像可以降低直接在生产环境进行变更所带来的风险。',
  RATIO_MODIFY_NOTIFY_CONTENT: '您已将 {version} 版本的目标流量配比调整至 {ratio}%，您可以继续调整目标流量配比，或者使它立即生效。',
  CANARY_BY_TRAFFIC_DESC: '根据流量比例，对组件 {component} 的 {ratio}% 的请求流量会转发到灰度版本 {newVersion}。',
  ...
  PREREQUEST_FOR_USE_GRAYRELEASE_A: '使用灰度发布功能前，您需要创建自制应用并且开启应用治理功能。',
  JOB_OFFLINE_WARNING: '您需要选择一个下线版本，系统会保留一个可用版本并且自动将流量全部切换至该可用版本，保证服务正常运行。',
